<!--
  ...
  Modified principles:
    - II. Code Style Standards → Added docstring imperative mood requirement, ESLint/Prettier integration guidance
    - IV. Testing & Quality Assurance → Added function coverage requirement, Vue component testing guidance
    - VI. Pre-Commit Compliance → Expanded with linter order requirements and conflict resolution
  Added sections: None
  ...
  Amendment Rationale:
  - Addresses pre-commit failures caused by ESLint/Prettier conflicts
  - Prevents docstring D401 violations with explicit imperative mood requirement
  - Ensures adequate test coverage by requiring function coverage checks
  - Improves Vue component test quality with stubbing best practices
-->

# Weni Roadmap Constitution
...
- Use BEM methodology for CSS classes: `.block__element--modifier`
...
- Frontend MUST use ESLint, Prettier, and Stylelint for consistent code quality
...
- Mock Unnnic components properly to render slot content: `UnnnicModal: { template: '<div><slot /></div>' }`
...
- For lifecycle hooks (`watch`, `onMounted`), verify they trigger expected behavior
- Avoid `any` types in test code—use proper TypeScript types for test helpers
- Rationale: Proper component mocking prevents "slot content not rendered" test failures
...
- Avoid inline styles; use external stylesheets with BEM methodology
...
2. Run formatters in this EXACT order (prevents conflicts):
   ...
   - **Frontend**:
     ...
     4. `npm run stylelint:check` (verify CSS rules)
...
**Common Pre-Commit Errors & Solutions**:

| Error | Cause | Solution |
|-------|-------|----------|
| `F541 f-string is missing placeholders` | Using `f"text"` without `{var}` | Change to regular string: `"text"` |
| `F401 imported but unused` | Unused import statement | Remove the import or use it |
| `D401 First line should be in imperative mood` | Docstring starts with present tense | Use imperative: "Return" not "Returns" |
| `trailing-whitespace` | Spaces/tabs at end of lines | Let pre-commit auto-fix, then re-commit |
| `end-of-file-fixer` | Missing newline at end of file | Let pre-commit auto-fix, then re-commit |
| `Expected indentation of N spaces` | ESLint/Prettier conflict | Run `npm run format` then `npm run lint -- --fix` |
| `Expected no line breaks before closing bracket` | Vue ESLint/Prettier conflict | Move `eslint-config-prettier` to END of ESLint config |
| `no-explicit-any` | TypeScript `any` type used | Use `unknown` with type guards or specific types |
| `no-unused-vars` | Unused function parameter | Prefix with underscore: `_unusedParam` |
| `Coverage for functions (X%) does not meet threshold (80%)` | Functions not called in tests | Add tests that actually call all exported functions |
| `Expected modern color-function notation` | Using `rgba(...)` syntax | Use modern syntax: `rgb(R G B / alpha%)` |
| `Unexpected vendor-prefixed property` | Using `-webkit-` or `-moz-` | Remove vendor prefixes; use standard property only |
...
├── styles/          # Global styles and preprocessor configs
├── services/        # Service logic and API integration
│   └── api/         # API calls and data models
...
- Keep template logic minimal—move complex conditionals to computed properties
- Use lowercase for directory and file names
- Avoid excessive folder nesting (reduces cognitive complexity)

**Component Guidelines**:
- Name components descriptively with scope prefixes (e.g., `RoadmapCard`, `AppHeader`)
...
This constitution supersedes all other coding practices within the Weni Roadmap project. All team members and contributors MUST comply.
...
**Version**: 1.2.0 | **Ratified**: 2025-12-26 | **Last Amended**: 2026-01-21
